...
const ToastClose = React.forwardRef<HTMLButtonElement, ToastCloseProps>(
  ({ className, onClick, ...props }, ref) => (
    <button
      ref={ref}
      type="button"
      onClick={onClick} // ✅ onClick is now passed through
      className={cn(
        // always visible (removed opacity-0), bigger click target
        "absolute right-2 top-2 rounded-md p-1.5 text-foreground/60 transition-all hover:text-foreground hover:bg-black/10 focus:opacity-100 focus:outline-none focus:ring-2 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600 cursor-pointer",
        className,
      )}
      toast-close=""
      {...props}
